...
if(isset($_POST) && $_POST['submit']=='adddata') {
	...
	//echo "INSERT INTO `gallery`(`name`, `image`,`nameA`) VALUES ('$name','$image','$nameA')";exit;
	$sth = $db->query("INSERT INTO `gallery`(`name`, `image`) VALUES ('$name','$image')");
	if($sth == true) { ?>
	<script type="text/javascript">
			alert('Data Successfully Inserted');
			window.location="<?php echo URL; ?>gallery.php";
			</script>
	<?php } else { ?>
	<script type="text/javascript">
...
else if(isset($_POST) && $_POST['submit']=='editdata')
{
	$name = $_POST['titlename'];
	$filename = stripslashes($_FILES['images1']['name']);
	if(!empty($filename)) {
		...
		$img_path="../adminupload/".$image;
		move_uploaded_file($_FILES['images1']['tmp_name'], $img_path);
		$ith = $db->query("UPDATE `gallery` SET `image`='$image' WHERE `guid`='$qid'");
	} 
	//echo "UPDATE `gallery` SET `name`='$name' ,`nameA`='$nameA' WHERE `guid`='$qid'";exit;
	$sth = $db->query("UPDATE `gallery` SET `name`='$name' WHERE `guid`='$qid'");
	if($sth == true) { ?>
	<script type="text/javascript">
	...
	//print_r($_POST); exit;
	 $d = $_POST['id'];
		    $del1 = $db->query("SELECT `image` FROM `album` WHERE `aid`='".$d."'"); //Delete query for Galley Album Images.
			$row1 = $del1->fetch();
			$filepath1 = "../adminupload/".$row1['image'];
			if(file_exists($filepath1)) {
				unlink($filepath1);
			}
			$del1= $db->query("DELETE FROM `album` WHERE `aid`='".$d."'");
			
			
			$del = $db->query("SELECT `image` FROM `gallery` WHERE `guid`='".$d."'");
			$row = $del->fetch();
			$filepath = "../adminupload/".$row['image'];
			if(file_exists($filepath)) {
				unlink($filepath);
			}
			$sth = $db->query("DELETE FROM `gallery` WHERE `guid`='".$d."'");
			if($sth == true) {
			  echo "1111";	 exit;
			} 
 }
      ...
      <div class="col-lg-12"> 
        ...
        <div class="breadcrumbs">
            <ul class="breadcrumb">
              ...
              <li class="active"><a href="<?php echo URL; ?>gallery.php">Gallery</a></li>
            </ul>
              ...
              <ul>
                ...
                <li class="dropdown"><a href="#" title="" data-toggle="dropdown"><i class="fa fa-plus"></i><span> Tasks</span></a>
                  <ul class="dropdown-menu dropdown-primary dropdown-menu-right">
                    <?php // if($count < 10) {?>
                    <li><a href="<?php echo URL; ?>gallery.php?action=add">Add New Gallery</a></li>
                    <?php //} ?>
                    <li><a href="<?php echo URL; ?>gallery.php">View Gallery</a></li>
                  </ul>
                </li>
              </ul>
            </div>
        ...
        <div class="page-header title"> 
          <!-- PAGE TITLE ROW -->
          <h1>Gallery <!--<span class="sub-title">responsive photo gallery using colorbox</span>--></h1>
        </div>
...
<a href="<?php echo URL; ?>gallery.php" class="btn btn-info">Back</a>
    <div class="row">
      <div class="col-lg-8"> 
        <!--portlet start--->
        <div class="portlet">
            ...
            <div class="portlet-body">
              <form role="form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" enctype="multipart/form-data">
                <div class="form-group">
                  <label>Name</label>
                  <input type="text" class="form-control" name="titlename" placeholder="Enter Text Here.." required>
                </div>
				<!--<div class="form-group">
                  <label>Location</label>
                  <input type="text" class="form-control" name="loc" placeholder="Enter Text Here.." required>
                </div>-->
				<div class="form-group">
                  <label>Image</label>
                  <input type="file" class="form-control" required name="images1">
                </div>
    ...
    <?php } 
	   ...
	   $sth = $db->query ('SELECT * FROM `gallery` WHERE `guid`="'.$q.'"');
	   $row = $sth->fetch();
   ?>
<a href="<?php echo URL; ?>gallery.php" class="btn btn-info">Back</a>
    <div class="row">
          ...
          <div id="f-12" class="panel-collapse collapse in" aria-expanded="true">
            <div class="portlet-body">
              <form role="form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" enctype="multipart/form-data">
                <div class="form-group">
                  <label>Name</label>
                  <input type="text" class="form-control" name="titlename" placeholder="Enter Text Here.." value="<?php echo $row['name']; ?>">
                </div>
				
				<!--<div class="form-group">
                  <label>Location</label>
                  <input type="text" class="form-control" name="loc" placeholder="Enter Text Here.." value="<?php //echo $row['loc']; ?>">
                </div>-->
                ...
                <div class="form-group">
                  ...
                  <input type="image" class="img-responsive" src="<?php if(empty($row['image'])) { echo URL.'assets/images/gallery/img1.jpg'; } else { echo URL.'../adminupload/'.$row['image']; } ?>" width="150px" height="150px"/>
                </div>
                <div class="form-actions">
    ...
    <?php } else { ?>
<a href="<?php echo URL; ?>gallery.php?action=add" class="btn btn-info">Add New</a>
    <div class="row">
      <div class="col-lg-12">
        <div class="portlet">
          <div class="portlet-heading dark">
          ...
          <div class="padding-25 panel-collapse collapse in" id="f-1" aria-expanded="true"> 
            ...
            <ul class="tc-gallery-2 clearfix">
              <?php 
			 $sth = $db->query ('SELECT * FROM `gallery`');
			 $count = $sth->rowCount();
			 if($count > 0) {
				 $m= 1;
				 while($row = $sth->fetch()) { ?>
              <li class="thumbnail thumbnail-box-<?php echo $row[0]; ?>">
                <div class="thumb-preview">
                  ...
                  <div class="gl-thumb-options"> <a class="gl-zoom" href="<?php if(empty($row[2])) { echo URL.'assets/images/gallery/img1.jpg'; } else { echo URL.'../adminupload/'.$row[2]; } ?>" data-rel="colorbox" title="<?php echo $row['name']; ?>">  <i class="fa fa-search"></i> </a>
                    <div class="gl-toolbar"> 
                      ...
                      <div class="gl-group pull-right"> <a href="<?php echo URL; ?>gallery.php?action=edit&q=<?php echo $row[0]; ?>">Edit</a>
                ...
                <div class="gl-description"><small class="pull-left"><a href="<?php echo URL; ?>album.php?album=<?php echo $row[0]; ?>" rel="nofollow"><?php echo $row[1]; ?></a></small></div>
              </li>
              <?php $m++; } } else {?>
...
<script type="text/javascript">	
	...
	function deleteme(s){
		...
		$.ajax({
				  ...
				  dataType:"text",
				  data:query,
				  url:"<?php echo URL; ?>gallery.php",// url of php page where you are writing the query
