...
if(isset($_POST) && $_POST['submit']=='adddata') {
	...
	foreach($combine_array as $tmpname=>$name) {
		...
		$sth = $db->query("INSERT INTO `p_album`(`aid`, `image`) VALUES ('$aid','$image')");
	}
	if(is_array($combine_array))
	{ ?>
	<script type="text/javascript">
			alert('Data Successfully Inserted');
			window.location="<?php echo URL; ?>s_cat_album.php?album=<?php echo $aid; ?>";
			</script>
	<?php }  else { ?>
	<script type="text/javascript">
...
else if(isset($_POST) && $_POST['submit']=='editdata')
{
	...
	if(!empty($filename)) {
		...
		$ith = $db->query("UPDATE `p_album` SET `image`='$image' WHERE `guid`='$qid'");
	} 
	if($ith == true) { ?>
	<script type="text/javascript">
	...
	//print_r($_POST); exit;
	 $d = $_POST['id'];
		    $del = $db->query("SELECT `image` FROM `p_album` WHERE `guid`='".$d."'");
			$row = $del->fetch();
			$filepath = "../adminupload/".$row['image'];
			if(file_exists($filepath)) {
				unlink($filepath);
			}
			$sth = $db->query("DELETE FROM `p_album` WHERE `guid`='".$d."'");
			if($sth == true) {
			  echo "1111";	 exit;
			} 
 }
    ...
    <div class="row">
      <div class="col-lg-12"> 
        ...
        <div class="breadcrumbs">
		    <?php 
			    //echo "SELECT * FROM `sub_cat` WHERE `guid`='$aid'";exit;
				$a= $db->query("SELECT * FROM `sub_cat` WHERE `guid`='$aid'");
				while($b= $a->fetch()){$x= $db->query("SELECT * FROM `sub_cat` WHERE `guid`='".$b['cid']."'"); $y=$x->fetch(); }?>
            <ul class="breadcrumb">
              <li> <a href="<?php echo URL; ?>home.php">Home</a> </li>
              <li class="active"><a href="<?php echo URL; ?>s_cat.php"><?php echo $y['name']; ?></a></li>
              <li class="active"><a href="<?php echo URL; ?>s_cat_album.php?album=<?php echo $aid; ?>">Album</a></li>
            </ul>
              ...
              <ul>
                ...
                <li class="dropdown"><a href="#" title="" data-toggle="dropdown"><i class="fa fa-plus"></i><span> Tasks</span></a>
                  <ul class="dropdown-menu dropdown-primary dropdown-menu-right">
                    <?php // if($count < 10) {?>
                    <li><a href="<?php echo URL; ?>s_cat_album.php?action=add&album=<?php echo $aid; ?>">Add New</a></li>
                    <?php //} ?>
                    <li><a href="<?php echo URL; ?>s_cat_album.php?album=<?php echo $aid; ?>">View Album</a></li>
                  </ul>
                </li>
              </ul>
            </div>
        ...
        <div class="page-header title"> 
          <!-- PAGE TITLE ROW -->
          <h3><?php echo $y['name']; ?>  <span class="sub-title">Album</span></h3>
        </div>
...
<a href="<?php echo URL; ?>s_cat_album.php?album=<?php echo $aid; ?>" class="btn btn-info">Back</a>
    <div class="row">
      <div class="col-lg-8"> 
        <!--portlet start--->
        <div class="portlet">
    ...
    <?php } 
	   ...
	   $sth = $db->query ('SELECT * FROM `p_album` WHERE `guid`="'.$q.'"');
	   $row = $sth->fetch();
   ?>
<a href="<?php echo URL; ?>s_cat_album.php?album=<?php echo $aid; ?>" class="btn btn-info">Back</a>
    <div class="row">
    ...
    <?php } else { ?>
<a href="<?php echo URL; ?>s_cat_album.php?action=add&album=<?php echo $aid; ?>" class="btn btn-info">Add New Albums</a>
        ...
        <div class="portlet">
          ...
          <div class="padding-25 panel-collapse collapse in" id="f-1" aria-expanded="true"> 
            ...
            <ul class="tc-gallery-2 clearfix">
              <?php 
			 $sth = $db->query ("SELECT * FROM `p_album` WHERE `aid`={$aid} ORDER BY `guid` DESC");
			 $count = $sth->rowCount();
			 if($count > 0) {
				 $m= 1;
				 while($row = $sth->fetch()) { ?>
                ...
                <div class="thumb-preview">
                  ...
                  <div class="gl-thumb-options"> <a class="gl-zoom" href="<?php if(empty($row[2])) { echo URL.'assets/images/gallery/img1.jpg'; } else { echo URL.'../adminupload/'.$row[2]; } ?>" data-rel="colorbox" title="<?php echo $row[1]; ?>"> <i class="fa fa-search"></i> </a>
                    <div class="gl-toolbar"> 
                      ...
                      <div class="gl-group pull-right"> <a href="<?php echo URL; ?>s_cat_album.php?action=edit&q=<?php echo $row[0]; ?>&album=<?php echo $aid; ?>">Edit</a>
...
<script type="text/javascript">	
	...
	function deleteme(s){
		...
		$.ajax({
				  ...
				  dataType:"text",
				  data:query,
				  url:"<?php echo URL; ?>s_cat_album.php",// url of php page where you are writing the query
